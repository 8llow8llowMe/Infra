...
# 지원: Ubuntu 20.04, 22.04, 24.04 / 아키텍처: amd64 (자동 감지)

set -e

echo "========================================="
echo "Docker Engine 설치 스크립트 (일반/amd64)"
echo "========================================="
echo ""
...
if [ "$EUID" -ne 0 ]; then
    echo "root 권한이 필요합니다."
    echo "다음 명령으로 실행하세요: sudo ./install-docker.sh"
    exit 1
fi
...
# 아키텍처 확인 (amd64 권장, arm64는 install-docker-arm64.sh 사용)
ARCH=$(dpkg --print-architecture)
if [ "$ARCH" != "amd64" ]; then
    echo "현재 아키텍처: $ARCH"
    echo "arm64 서버는 install-docker-arm64.sh 를 사용하세요."
